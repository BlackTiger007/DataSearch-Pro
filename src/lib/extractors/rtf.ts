...
export async function extract(
	...
	const raw = createTextDecoder().decode(uint8Array);

	const text = convertToPlain(raw);
...
function convertToPlain(rtf: string) {
	...
	let curskip = 0;
	const out = [];
	let match, word, arg, hex, char, brace, tchar;
	while ((match = pattern.exec(rtf))) {
		...
		if (brace) {
			...
			if (destinations.has(word)) {
				...
				out.push(specialchars[word as keyof typeof specialchars]);
			} else if (word === 'uc') {
				ucskip = parseInt(arg);
			} else if (word === 'u') {
				let c = parseInt(arg);
